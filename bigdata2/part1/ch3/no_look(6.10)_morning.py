...
# # value_counts()를 아예 빼먹음! 이걸 해야 빈도수 많은 날짜를 구할 수 있음...
# # exam2['subscribed'] = pd.to_datetime(exam2['subscribed'])
# # print(exam2['subscribed'].head())
...
# # print(exam4[:10])
# top10 = exam4['views'].iloc[9]
# # print(top10)
# # print(exam4['views'].iloc[:9])
# exam4['views'].iloc[:9] = top10
# # print(exam4['views'].iloc[:9])
# sum = exam4['views'].sum()
# print(int(sum))

# # 총평: print 연발하고 고민/확인을 거듭한 끝에 드디어 혼자 풀었다!! 
...
# # print(exam19.head(30))
# exam19 = exam19.bfill()
# # print(exam19)
# sum = exam19.groupby(['city', 'f2']).sum(numeric_only=True).reset_index() # .reset_index()!!
# # print(sum)
# #### numeric_only=True까지는 떠올렸으나, 그 다음이 문제!!
# sum = sum.sort_values('views', ascending=False) # 그룹합을 구한 다음 내림차순 정렬을 해줘야!
# # print(sum)

# # 그런 다음 3번째로 큰 값 출력
# print(sum.iloc[2,0])

# ### 총평: 그룹합 구하고 numeric_only=True까지 간건 잘했으나, 이걸 sort_values해서 iloc로 찾는 건 실패!
...
# # print(int(round(mean.iloc[0]))) # min을 해도 돼지!!
...
# # print(exam36)
# ##### transform을 잘못 썼나?
# # exam36['판매금액'] = exam36['판매금액'].fillna(exam36['판매금액'].transform('mean'))
# # print(exam36)
# ##### 완전 잘못 씀...
...
# # # print(exam13)
# # nan = exam13[exam13.isna()==True]
# # print(nan) 결측치 있는 데이터만 조회... 여기서 완전 뜬 구름 잡았다!!
