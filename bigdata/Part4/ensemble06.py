...
9절. 앙상블
 ...
 6. LightGBM : LightGBM은 lgihtgbm 패키지를 설치한 후 해당 사이킷런 래퍼 패키지 내 클래스 LGBMClassifer, LGBMRegressor 등을 통해 모형객체를 생성
 한다. 클래스들의 사용법은 아래와 같으며 LGBMClassifier는 이진분류와 다지분류, LGBMRegressor는 연속형에 사용된다.
 
lighbtgbm.LGBMClassifier(boosting_type='gbdt',max_depth = -1, n_estimators = 100, n_jobs = None, objective = None, ...)
...
# 모델학습
model_lgb_wrap_bin = lgb_wrap_bin.fit(X_train, y_train, ) # eval_metric = 'mlogloss' 여기서 제거!

"""
 - breast_cancer 데이터셋을 8대 2의 비율로 학습 데이터와 평가 데이터로 분할한 후 사이킷런 래핑 lightgbm 모형객체를 생성한 후 모형을 학습하였다.
"""
...
"""
 - AUC를 계산한 결과 0.9997으로 매우 높게 나타났다.
 - 이 결과는 무조건 높다고 해서 좋은 모형은 아니며, 분석 분야에 따라 다양한 지표들을 활용하여 분석 모형을 선택할 수 있음을 참고하자.
"""

"""
Q. 사이킷런 패키지 내 iris 데이터를 호출한 후 학습 데이터와 평가 데이터로 분할하고 클래스 LGBMClassifier()를 통해 다지분류 모형객체를 생성하고 
...
"""
 ...
 lgihtgbm.LGBMRegressor(boosting_type = 'gbdt', max_depth = -1, n_estimators = 100, n_jobs = None, learning_rate = 0.1, 
  objective = None, ...) 
"""

"""
Q. 사이킷런 패키지 내 diabetes 데이터를 호출한 후 학습 데이터와 평가 데이터로 분할하고 클래스 LGBMRegressor()를 통해 연속형 예측 모형객체를 
