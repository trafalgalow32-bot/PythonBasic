...
9절. 앙상블
 ...
 4. Gradient Boosting : Gradient Boosting은 사이킷런의 서브패키지 ensemble 내 클래스 GradientBoostingClassifier(), 
 GradientBoostingRegressor() 등을 통해 모형객체를 생성할 수 있다. 클래스들의 사용법은 아래와 같으며 GradientBoostingClassifier는 이진분류와
 다지분류, GradientBoostingRegressor는 연속형에 사용된다.
 
sklearn.ensemble.GradientBoostingClassifier(n_estimators = 100, criterion = 'friedman_mse', max_depth=3, min_samples_leaft=1,
max_features=None, learning_rate = 0.1, loss='log_loss' ...)

Q. 사이킷런 패키지 내 breast_cancer 데이터를 호출한 후 학습 데이터와 평가 데이터로 분할하고 클래스 GradientBoostingClassifier()를 통해 
...
"""
- breast_cancer 데이터셋을 8대 2의 비율로 학습 데이터와 평가 데이터로 분할한 후 Gradient Boosting 모형객체를 생성한 후 모형을 학습하였다.
"""
...
"""
Q. 사이킷런 패키지 내 iris 데이터를 호출한 후 학습 데이터와 평가 데이터로 분할하고 클래스 GradientBoostingClassifier()를 통해
...
"""
iris 데이터셋을 8대 2의 비율로 학습 데이터와 평가 데이터로 분할한 후 GradientBoosting 모형객체를 생성한 후 모형을 학습하였다.
"""
...
"""
 ...
 sklearn.ensemble.GradientBoostingRegressor(n_estimators = 100, criterion='friedman_mse', max_depth = 3, min_samples_leaf = 1,
 max_features = None, learning_rate = 0.1, loss = 'squared_error', ...)
"""

"""
Q. 사이킷런 패키지 내 diabetes 데이터를 호출한 후 학습 데이터와 평가 데이터로 분할하고 클래스 GradientBoostingRegressor()를 통해 연속형 예측 
