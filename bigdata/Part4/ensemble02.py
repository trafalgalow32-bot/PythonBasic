...
9절. 앙상블
 ...
 2. 랜덤포레스트 : 랜덤포레스트는 사이킷런의 서브패키지 ensemble 내 클래스 RandomForestClassifier(), RandomForestRegressor() 등을 통해
 모형객체를 생성할 수 있다. 클래스들의 사용법은 아래와 같으며, RandomForestClassifier는 이진분류와 다지분류, RandomForestRegressor는 연속형
 타겟변수에 사용된다.
sklearn.ensemble.RandomForestClassifier(n_estimator = 100, criterion = 'gini', max_depth = None, min_samples_leaf = 1, max_features = 'sqrt', 
...)

Q. 사이킷런 패키지 내 breast_cancer 데이터를 호출한 후 학습 데이터와 평가 데이터로 분할하고 클래스 RandomForestClassifier()를 통해 
...
"""
- breast_cancer 데이터셋을 8대 2의 비율로 학습 데이터와 평가 데이터로 분할한 후 랜덤포레스트 모형객체를 생성한 후 모형을 학습하였다.
"""
...
"""
Q. 사이킷런 패키지 내 iris 데이터를 호출한 후 학습 데이터와 평가 데이터로 분할하고 클래스 RandomForestClassifier()를 통해
...
"""
iris 데이터셋을 8대 2의 비율로 학습 데이터와 평가 데이터로 분할한 후 랜덤포레스트 모형객체를 생성한 후 모형을 학습하였다.
"""
...
"""
 ...
 sklearn.ensemble.RandomForestRegressor(n_estimator = 100, criterion = 'squared_error', max_depth = None, min_samples_leaf = 1,
  max_features = 1.0, ...)
"""

"""
Q. 사이킷런 패키지 내 diabetes 데이터를 호출한 후 학습 데이터와 평가 데이터로 분할하고 클래스 RandomForestRegressor()를 통해 연속형 예측 
